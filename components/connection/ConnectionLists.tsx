...
const ConnectionLists = ({
  ...
  return (
    <View style={styles.container}>
      <AppText style={styles.nick}>
        보낸 유저 : {requester.nickname} ({requester.id})
      </AppText>
      <AppText style={styles.meta}>
        보낸 시간 : {new Date(createdAt).toLocaleString()}
      </AppText>
      <AppText style={styles.meta}>해당 요청 목록 id : {id}</AppText>
      <AppText style={styles.meta}>요청 한 사람 id : {requesterId}</AppText>
      <AppText style={styles.meta}>
        요청 받은 사람 (나) id : {addresseeId}
      </AppText>
      <AppText style={styles.meta}>응답상태값 : {status}</AppText>

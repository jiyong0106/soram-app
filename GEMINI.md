...
5.  **연결 및 소통:**
    - 상대방이 '대화 요청'을 수락하면, 1:1 채팅방이 열린다.
...
- **음성 탐색:** 특정 주제에 대해 다른 사용자의 음성 답변을 듣는 기능. **콘텐츠 가치 보존과 재방문 유도를 위해 주제별로 일일 탐색 횟수가 제한된다. 또한, 더 나은 사용자 경험을 위해 이미 `PENDING` 또는 `ACCEPTED` 상태로 연결된 사용자의 답변은 탐색 목록에서 제외된다.**
...
- **Cache/Data Store:** Redis
  ...
  - **콘텐츠 소비 이력 추적:**
    - **일일 탐색 제한:** 사용자가 하루 동안 조회한 음성 답변 ID를 Set 자료구조에 기록(24시간 TTL)하여, 주제별 일일 탐색 횟수 제한 기능을 구현하는 데 사용됩니다.
    - **추가 답변 조회 기록:** 사용자가 재화를 소모하여 특정 유저의 다른 답변을 조회한 이력을 영구적으로 기록하여, 중복 조회를 방지하고 조회 가능한 답변 수를 제어하는 데 사용됩니다.
...
`database-schema.md`에 명시된 "사용자가 재화를 소모하기 전 상대방의 동의를 얻어 부정적인 경험을 방지한다"는 핵심 UX 원칙에 따라, 채팅 기능은 **'연결(Connection) 관리'**와 **'실시간 메시징'** 두 단계로 나누어 구현한다.

#### 1단계: 연결 관리 (Connection Management via REST API)

사용자 간의 '대화 요청-수락/거절/취소' 과정을 관리하는 REST API 계층. `Connection`의 생명주기(`PENDING` -> `ACCEPTED` / `REJECTED` / `CANCELED`)를 명확히 제어한다.

- **모듈**: `ConnectionsModule` (`connections.controller.ts`, `connections.service.ts`)
- **핵심 API 엔드포인트**:
  1.  `POST /connections/request`: **(대화 요청 보내기)**
      - `requesterId`(본인)와 `addresseeId`(상대방)로 `Connection` 레코드를 생성한다.
      - **(재화 예약)** 요청 시점에 요청자의 `(총 대화권 수) > (보류중인 요청 수)`인지 검사하여, 사용 가능한 재화가 있을 때만 요청을 허용한다.
      - `status`는 `PENDING`으로 설정되며, **이 단계에서는 실제 재화가 소모되지 않는다.**
      - 상대방에게 푸시 알림 등을 통해 요청이 왔음을 알린다.
      - **(대화의 진정성 확보)** 특정 `voiceResponseId`를 기반으로 대화를 요청하려면, 요청자 또한 **해당 음성 답변과 같은 주제에 대한 자신의 답변을 먼저 등록**해야만 한다. 이는 일방적인 소통이 아닌, 공통된 관심사에 대한 양방향 소통을 유도하는 핵심 규칙이다.
      - **(연결의 유일성 보장)** 사용자 간의 연결은 '사람'을 중심으로 하며, 한 번에 하나의 활성화된(`PENDING` 또는 `ACCEPTED`) 연결만 유지되는 것을 원칙으로 한다. 따라서 이미 상대방과 `PENDING` 또는 `ACCEPTED` 상태의 연결이 존재할 경우, 새로운 대화 요청을 보낼 수 없다. 이는 `voices` 모듈의 탐색 단계에서부터 해당 사용자의 답변을 노출시키지 않으며, `connections` 모듈의 요청 단계에서도 서버 측 검증을 통해 중복 연결을 원천적으로 차단한다. 이를 통해 사용자는 혼란 없이 현재의 연결에 집중하고, 새로운 만남을 탐색하도록 유도한다.
      - **(교차 요청 자동 수락)** 만약 상대방이 나에게 보낸 `PENDING` 상태의 요청이 이미 존재할 경우, 새로운 요청을 생성하는 대신 즉시 해당 요청을 수락 처리하여 연결을 성사시킨다.
      - **(중복 요청 방지)** 사용자가 아직 처리되지 않은 `PENDING` 상태의 요청을 동일한 상대방에게 중복으로 보낼 수 없도록 제한한다.
      - **(효율적인 재요청)** 사용자가 이전에 `REJECTED` 또는 `CANCELED` 처리된 요청과 동일한 `voiceResponseId`로 다시 요청을 보내면, 불필요한 데이터를 쌓지 않고 기존 연결 레코드의 상태를 `PENDING`으로 업데이트하여 처리한다.
  2.  `POST /connections/:connectionId/accept`: **(대화 요청 수락)**
      - 요청을 받은 사용자(`addresseeId`)가 호출한다.
      - `Connection`의 `status`를 `ACCEPTED`로 변경한다.
      - **이 시점에 요청자(`requesterId`)의 재화(티켓)를 차감하고, `Transaction`에 기록한다.**
      - 양쪽 사용자에게 연결이 성사되었음을 알린다.
  3.  `POST /connections/:connectionId/reject`: **(대화 요청 거절)**
      - 요청을 받은 사용자가 호출한다.
      - `Connection`의 `status`를 `REJECTED`로 변경한다. '예약'되었던 재화가 해제된다.
  4.  `POST /connections/:connectionId/cancel`: **(보낸 요청 취소)**
      - 요청을 보낸 사용자(`requesterId`)가 직접 호출한다.
      - `Connection`의 `status`를 `CANCELED`로 변경한다. '예약'되었던 재화가 해제된다.
  5.  `GET /connections`: **(내 채팅방 목록 조회)**
  ...
  6.  `GET /connections/pending`: **(받은 요청 목록 조회)**
      - 나에게 온 대화 요청 중, 아직 수락/거절하지 않은 `PENDING` 상태의 목록을 조회한다.
  7.  `GET /connections/sent`: **(보낸 요청 목록 조회)**
      - 내가 보낸 대화 요청 중, 상대방이 아직 처리하지 않은 `PENDING` 상태의 목록을 조회한다.
  8.  `DELETE /connections/:connectionId/leave`: **(채팅방 나가기)**
  ...
  9.  `GET /connections/:id/trigger`: **(채팅방의 연결 계기 조회)**
...
- **거절하기 (이번만 거절):**
  - **동작:** 사용자가 특정 `음성 답변`에 대한 대화 요청만 거절한다.
  - **결과:** 요청자는 해당 `음성 답변`으로는 다시 요청을 보낼 수 없지만, 거절한 사용자의 **다른 음성 답변**에는 새로 대화를 요청할 수 있다.
...
- **주요 로직**:
  ...
  4.  `GET /connections/:connectionId/messages`: **(이전 대화 내역 불러오기 - REST API)**
...
1.  `POST /auth/phone/request-otp`: 휴대폰 번호로 4자리 OTP를 요청하고, 서버는 이를 3분간 Redis에 저장한다.
...
- **핵심 기술:** Nest.JS `EventEmitterModule`
...
3.  **알림 처리 흐름:**
    - 리스너 함수는 이벤트와 함께 전달된 데이터를 받아 다음을 순차적으로 처리한다.
      1.  **알림 DB 저장:** 사용자에게 보여줄 메시지를 생성하여 `Notification` 테이블에 저장한다. 이는 앱 내 '알림 보관함' 기능을 위함이다.
      2.  **푸시 토큰 조회:** 알림을 받을 사용자의 `userId`로 `Device` 테이블에서 하나 이상의 푸시 토큰(`pushToken`)을 조회한다.
      3.  **푸시 알림 발송:** 조회된 푸시 토큰을 이용해 FCM/APN 등 외부 푸시 서비스에 메시지 발송을 요청한다.
...
- **안정성:** 외부 푸시 서비스의 장애가 `Connection` 생성과 같은 핵심 비즈니스 로직에 직접적인 영향을 미치지 않는다.
